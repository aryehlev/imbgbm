...
pub struct Config {
    ...
    // ── Column subsampling (feature fraction per tree) ────────────────────────
    /// Fraction of features sampled at random for each tree.  1.0 = all features.
    /// Values in (0, 1) reduce variance and speed up training (like LightGBM's
    /// `feature_fraction`).
